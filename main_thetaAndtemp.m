 ...
 for k=(t0+1):nt1
        ...
        for i=1:nz
            for j=1:nx
                ii=Number(i,j);
                if i==1
                    p1(j)=rhos(j)*g*etat(j,1); p2(j)=rhos(j)*g*etat(j,1)+abs(rho2(ii)*g*0.5*dz);
                else
                    p1(j)=p2(j);
                    p2(j)=p2(j)+abs(rho2(ii)*g*dz);
                end
       ...
       for ti=1:num_dt
        ...
        for j=1:nx
            for i=2:nz
                ...
                if j==1
                    ii2=Number(i,nx-1);
                    hdiv_inter(j)=hdiv_inter(j)-dt/2/dx*(u2(ii+1,1)-u2(ii2,1))*(z(i)-z(i-1));
                elseif j==nx
                    ii2=Number(i,2);
                    hdiv_inter(j)=hdiv_inter(j)-dt/2/dx*(u2(ii2,1)-u2(ii-1,1))*(z(i)-z(i-1));
                else
                    hdiv_inter(j)=hdiv_inter(j)-dt/2/dx*(u2(ii+1,1)-u2(ii-1,1))*(z(i)-z(i-1));
                end
            end
...
time(:)=time(:)+t0;
  ...
  %%
path='twod_Keplertaux2_CD1E-2_melt2000K_bd1E3_kh1000kz1E-4_Az1E2_restart9.nc';
