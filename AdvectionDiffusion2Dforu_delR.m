    ...
    function [Tt] = AdvectionDiffusion2Dforu_delR( nx, nz, dt, dx, delRc, kh, kz, ut1, wt1, pres1t, rho1t, taux, bottomDragLinear)
...
%      for i=1:nz-1
         for j=1:nx
              ...
              if (i>2) && (i<nz-1)
                   ...
                   sz2 = kz(ii2)*dt/4/delRc(i)/delRc(i);   
                   sz1 = kz(ii1)*dt/4/delRc(i)/delRc(i);    
                   sz = sz2+sz1; 
                  ...
                  if i==1
                      sx = 2*kh(ii)*dt/dx/dx;     sz = kz(ii)*dt/delRc(i)/delRc(i);
                      A(ii,ii) = 1+sx+sz;
                      A(ii, Number(i+1,j)) = -kz(ii)*dt/delRc(i)/delRc(i);
                  elseif i==nz
                      sx = 2*kh(ii)*dt/dx/dx;     sz = kz(ii)*dt/delRc(i-1)/delRc(i-1);
                      A(ii,ii) = 1+sx+sz+bottomDragLinear*dt/dz_s;
                      A(ii, Number(i-1,j)) = -kz(ii)*dt/delRc(i-1)/delRc(i-1);
                  else
                    sx = 2*kh(ii)*dt/dx/dx;     sz = 2*kz(ii)*dt/delRc(i)/delRc(i);
                    A(ii,ii) = 1+sx+sz;
                    A(ii, Number(i-1,j)) = -kz(ii)*dt/delRc(i)/delRc(i);
                    A(ii, Number(i+1,j)) = -kz(ii)*dt/delRc(i)/delRc(i);
                  end
              ...
              if i==1
                  ii2 = Number(i+1,j);
                  Gt2(ii,1) =  -wt1(ii)*(ut1(ii2)-ut1(ii))/delRc(i) + Fu(j);   %surface wind forcing
              elseif i==nz
                  ii1 = Number(i-1,j);
                  Gt2(ii,1) =  -wt1(ii)*(ut1(ii)-ut1(ii1))/delRc(i-1);   %bottom linear drag
              else
                  ii2 = Number(i+1,j);
                  ii1 = Number(i-1,j);
                  Gt2(ii,1) =  -wt1(ii)*(ut1(ii2)-ut1(ii1))/2/delRc(i);
              end
